...
const findById = (id) => {
	return new Promise((resolve, reject) => {
		db.query(
			"SELECT tickets.*, airlines.airline_name, airlines.class_category FROM tickets LEFT JOIN airlines ON tickets.airline_id = airlines.airline_id WHERE ticket_id = $1",
			[id],
			(err, result) => {
				if (err) return reject(err);
				resolve(result);
